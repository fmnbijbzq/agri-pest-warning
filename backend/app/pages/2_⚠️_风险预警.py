...
from app.ui_style import inject_css, page_header, section, stat_card_row, risk_banner, svg_icon
...
with col2:
    ...
    temperature = st.slider("平均气温 (°C)", -5.0, 45.0, 25.0, 0.5)
with col3:
    humidity = st.slider("平均湿度 (%)", 20.0, 100.0, 70.0, 1.0)
    rainfall = st.slider("近期降雨 (mm)", 0.0, 100.0, 15.0, 1.0)
...
st.markdown(
    ...
    f'<span>{svg_icon("thermometer", 16, "#059669")} <b>{temperature}°C</b></span>'
    f'<span style="color:#e5e7eb;">|</span>'
    f'<span>{svg_icon("droplets", 16, "#059669")} <b>{humidity}%</b></span>'
    f'<span style="color:#e5e7eb;">|</span>'
    f'<span>{svg_icon("cloud-rain", 16, "#059669")} <b>{rainfall}mm</b></span>'
    f'<span style="color:#e5e7eb;">|</span>'
    f'<span>{svg_icon("calendar", 16, "#059669")} <b>{month}月</b></span>'
    f'</div></div>',
...
if st.button("开始预测", type="primary", use_container_width=True):
    ...
    stat_card_row([
        ("bar-chart", f"{score:.1%}", "风险评分"),
        ("shield", level, "风险等级"),
        ("thermometer", f"{temperature}°C", "当前气温", f"湿度 {humidity}%"),
        ("cloud-rain", f"{rainfall}mm", "近期降雨"),
    ])

    st.markdown("")

    # ── 建议 ──
    section("防治建议", "lightbulb")

    if score >= 0.6:
        st.error(
            f"**{level}** — 当前气象条件下 **{province}** 的 **{crop}** "
