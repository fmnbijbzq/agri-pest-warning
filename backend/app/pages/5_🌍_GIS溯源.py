...
with f4:
    map_mode = st.selectbox("显示模式", ["热力图", "散点图"])
...
stat_card_row([
    ("clipboard", f"{len(month_data):,}", "当月记录", f"{month_sel}月"),
    ("bar-chart", f"{len(cumulative):,}", "累计记录", f"1-{month_sel}月"),
    ("zap", f"{avg_risk:.1%}", "平均风险", "当月"),
    ("map-pin", str(month_data["province"].nunique()), "涉及省份"),
    ("bug", str(month_data["pest_name"].nunique()), "病虫害种类"),
])
...
with map_col:
    ...
    m = folium.Map(location=[32.0, 112.0], zoom_start=5, tiles=None)
    folium.TileLayer(
    ...
    if map_mode == "热力图" and len(month_data) > 0:
        ...
        if heat_data:
            HeatMap(
                heat_data, radius=35, blur=25, max_zoom=8,
    ...
    for _, row in prov_stats.iterrows():
        ...
        if map_mode == "散点图":
            folium.CircleMarker(
                ...
                color=color, fill=True, fill_color=color, fill_opacity=0.6, weight=2,
            ...
            folium.CircleMarker(
                ...
                color="#1f2937", fill=True, fill_color="#ffffff", fill_opacity=0.9, weight=1,
                popup=folium.Popup(popup_html, max_width=250),
...
with panel_col:
    ...
    if len(month_data) > 0:
        ...
        st.markdown(
            f'<div class="white-card" style="font-size:0.83rem;line-height:1.8;color:#374151;">'
            ...
            f'{"<br>其中 <b style=color:#dc2626;>" + str(n_high) + " 个省份</b>处于中高风险区间。" if n_high > 0 else "<br>整体风险可控。"}'
            f'</div>',
    ...
    if len(prov_stats) > 0:
        ...
        for i, (_, row) in enumerate(top_provinces.iterrows()):
            ...
            st.markdown(
                ...
                f'<span style="color:#4b5563;font-weight:500;"><b>{i+1}.</b> {row["province"]}</span>'
    ...
    if len(month_data) > 0:
        ...
        for sev in ["重", "中", "轻"]:
            ...
            st.markdown(
                ...
                f'<div style="height:100%;width:{pct}%;background:{c};border-radius:3px;'
                f'transition:width 0.4s ease;"></div>'
                f'</div></div>',
                unsafe_allow_html=True,
            )
